Shader "Custom/AdLitTelephone" {
	...
	SubShader {
		...
		Pass {
			...
			TEXTURE2D(_SecondMap); SAMPLER(smapler_secondMap);
			...
			void InitalizeSurfaceData(Varyings IN, out SurfaceData surfaceData){
				...
				half4 secondMap = SAMPLE_TEXTURE2D(_SecondMap, sampler_BaseMap, IN.uv);
				
				baseMap = lerp(baseMap, secondMap, _Transition);
			...
			half3 LightLoop(SurfaceData surface_data, InputData inputData)
			{
				BRDFData data;
				InitializeBRDFData(surface_data, data);
				float3 lighting = 0;
			...
			half4 LitPassFragment(Varyings IN) : SV_Target {
				...
				mask = 1-mask;
				color.a = mask;
				float col = (color.r + color.g + color.g)/3.0;
				color.rgb = lerp(color.rgb,float3(col, col, col),_Transition);
